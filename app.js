...
inventorNames = inventors.map((find) => {
  return { first: find.first, last: find.last };
})
...
inventorNamedAda = inventors.find((find) => {
  return find.first === 'Ada'
});
// Check your work:
...
firstLast = people.map((name) => {
  ...
  let [lastNa, firstNa] = name.split(', ');

  return `${firstNa} ${lastNa}`;
...
isEveryone19OrOlder = devs.every((old) => {
  return 2021 - old.year >= 19;
})
// Check your work:
